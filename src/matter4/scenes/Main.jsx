...
import createRotatingPlatform from '../resources/CreateRotatingPlatform';
...
class Main extends Scene {
  ...
  create() {
    ...
    map.getObjectLayer('Crates').objects.forEach((crateObject) => {
      const { x: inX, y: inY, width, height } = crateObject;

      // Tiled origin for coordinate system is (0, 1), but we want (0.5, 0.5)
      this.matter.add
        .image(inX + width / 2, inY - height / 2, 'block')
  ...
  onPlayerCollide({ gameObjectB }) {
    if (!gameObjectB || !(gameObjectB instanceof Tilemaps.Tile)) {
      return;
    }
  ...
  onPlayerWin() {
    ...
    for (let i = 0; i < 35; i += 1) {
      const x = this.player.sprite.x + Math.RND.integerInRange(-50, 50);
      const y = this.player.sprite.y - 150 + Math.RND.integerInRange(-10, 10);
